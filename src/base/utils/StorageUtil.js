    ...
    static clearAll() {
        try {
            ...
            for (let i = 0; i < storageKeysToNoClear.length; i += 1) {
                ...
                dataToNotClear[storageKeysToNoClear[i]] = StorageUtil.get(storageKeysToNoClear[i]);
            }
            localStorage.clear();
            for (let i = 0; i < storageKeysToNoClear.length; i += 1) {
                // eslint-disable-next-line no-await-in-loop
                 StorageUtil.set(storageKeysToNoClear[i], dataToNotClear[storageKeysToNoClear[i]]);
            }
        }
        catch (e) {
            console.error(e);
